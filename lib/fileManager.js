...
export function getSortedFileList(files) {
  ...
  // 특수 파일들 정렬: profile.html -> (문제이름).html -> input.txt -> style.css
  const profileHtml = [];
  ...
  // specialFiles에 있는 나머지 파일들 (input.txt, style.css) - 순서대로
  ...
  special.push(...profileHtml, ...problemHtmlFiles, ...otherSpecialFiles);
...
export function isCodeFile(filename) {
  ...
  // 특수 파일 제외
  if (filename === 'input.txt' || filename === 'style.css') {
    return false;
  }
